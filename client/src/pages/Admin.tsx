...
import { ArrowLeft, RefreshCw, Save, ShieldCheck, Trash2, Truck as TruckIcon, UserPlus, Users, X } from "lucide-react";
...
type AdminTruck = { id: string; fleet_number: string; registration: string; truck_type: string | null; status: "ready" | "inspection_due" | "out_of_service" };
...
function AdminWorkspace() {
  ...
  const [truckForm, setTruckForm] = useState({ fleet_number: "", registration: "", truck_type: "", status: "ready" as AdminTruck["status"] });
  ...
  const load = async () => {
    ...
    const [{ data: truckData, error: truckError }, { data: driverData, error: driverError }] = await Promise.all([
      supabase.from("trucks").select("id, fleet_number, registration, truck_type, status").order("fleet_number"),
  ...
  const saveTruck = async (event: React.FormEvent) => {
    ...
    if (!supabase) return;
    const payload = { fleet_number: truckForm.fleet_number.trim(), registration: truckForm.registration.trim().toUpperCase(), truck_type: truckForm.truck_type.trim() || null, status: truckForm.status };
    ...
    setTruckForm({ fleet_number: "", registration: "", truck_type: "", status: "ready" });
