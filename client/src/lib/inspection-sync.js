...
export async function submitInspection({ allowQueue = true, ...draft }) { const queuedDraft = buildInspectionDraft({ ...draft, queued: true, syncStatus: SYNC_STATES.savedOnDevice }); const offline = !driverSupabase || (typeof navigator !== "undefined" && !navigator.onLine); if (offline) { if (!allowQueue) throw new Error("The connection is still offline."); await saveInspectionDraft(queuedDraft); writeSyncMeta({ state: SYNC_STATES.savedOnDevice, pending: true, lastError: "Waiting for an internet connection." }); return { queued: true }; } try { writeSyncMeta({ state: SYNC_STATES.uploading, pending: true, lastError: null }); const result = await submitOnline(draft); writeSyncMeta({ state: SYNC_STATES.submitted, pending: false, lastSyncAt: new Date().toISOString(), lastError: null }); return result; } catch (error) { if (allowQueue && retryableError(error)) { const attempts = (draft.syncAttempts || 0) + 1; await saveInspectionDraft({ ...queuedDraft, syncAttempts: attempts, lastSyncError: readableError(error) }); writeSyncMeta({ state: SYNC_STATES.savedOnDevice, pending: true, lastError: readableError(error) }); return { queued: true }; } throw new Error(readableError(error)); } }
