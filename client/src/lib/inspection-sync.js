...
async function putDraft(value) { const db = await openDraftDb().catch(() => null); if (db) { await new Promise((resolve, reject) => { const request = db.transaction("drafts", "readwrite").objectStore("drafts").put(value, DRAFT_KEY); request.onsuccess = resolve; request.onerror = () => reject(request.error); }); db.close(); return; } const storage = browserStorage(); if (storage) storage.setItem(`${DRAFT_STORE}:${DRAFT_KEY}`, await serializeForStorage(value)); }
export async function saveInspectionDraft(draft) { const db = await openDraftDb().catch(() => null); if (db) { await putDraft(draft); return; } const storage = browserStorage(); if (storage) storage.setItem(`${DRAFT_STORE}:${DRAFT_KEY}`, await serializeForStorage(draft)); }
